...
class TeleNavigation():
    ...
    def __init__(self):
        ...
        # switch control for follower using press
        ...
        # Right now maximum speed is 0.667 m/s. 
        # After speed scale of 0.3, it is 0.3*0.67= 0.2 m/s
        self.speed_scale = 0.3
        self.obstacle_speed_scale = 0.15   
        ...
        # Follower arm teloperation variables
        self.leader_ns = "leader"
        self.follower_ns = "panda_follower"

        self.print_odom = False
        ...
        # Follower arm teleoperation functions
    ...
    def leader_motion(self):
        ...
        if self.goalReached:
            ...
            if not self.control_follower_franka: self.followerCmdVelPub.publish( desired_cmd_vel )
    ...
    def lidar_cb(self, msg):
        ...
        if np.mean(self.lidar_ranges) != 0 and np.mean(self.lidar_ranges) <= self.min_obstacle_distance: # m
            self.obstacle = True
            # print("Ostacle present")
            self.obstacle_haptic_force.append(self.leaderComplianceParams['task_haptic_x_force'])
        elif np.mean(self.lidar_ranges) != 0 and np.mean(self.lidar_ranges) <= self.object: # m
            self.object_found = True
            print("Object found")
        else:
            self.obstacle = False
    ...
    """Code for follower arm teleoperation"""
    ...
    def leader_joint_state_cb (self, msg ): 
        ...
        if self.control_follower_franka:
            self.save_leader_joints.append(self.leader_joint_state.position[:7])  
 
    def follower_joint_state_cb (self, msg ): 
        self.follower_joint_state = msg
        if self.control_follower_franka:
            self.save_follower_joints.append(self.follower_joint_state.position[:7])  
    ...
    def write_to_file(self):
        ...
        np.savez('/home/venkatesh/haptic_ws/src/thorvald_panda_description/script/real_haptic_joints_goal_4', reference_x = self.save_reference_x, \
            ...
            haptic_feedback_yaw = self.save_haptic_feedback_yaw, obstacle_haptic_force = self.obstacle_haptic_force, external_torques = self.external_torques, \
            leader_joint_states = self.save_leader_joints, follower_joint_states = self.save_follower_joints, time = time_array, goal = goal)
